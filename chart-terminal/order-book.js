...
// FILE: chart-terminal/order-book.js
// ACTION: REPLACE ENTIRE FILE (single part — ~240 lines, 460-line limit ke andar)
// NOTE: market-store.js ke fixed depth stream ({price,qty,total}, 0.01-rounded,
//       cross-guarded) ke saath designed. Koi doosri file modify mat karna.
// ══════════════════════════════════════════════════════════════════════════
(function () {
...
const RENDER_MIN_GAP_MS = 150; // throttle: max ~6-7 repaints/sec instead of every 100ms tick
const STALE_AFTER_MS = 15000; // no depth event for 15s => show stale banner
// ── Style ────────────────────────────────────────────────────────────
const style = document.createElement('style');
style.textContent = `
.ob-panel{background:var(--bg2);border:1px solid var(--border,rgba(255,255,255,0.08));border-radius:12px;padding:10px;display:flex;flex-direction:column;height:100%;box-sizing:border-box;}
.ob-header{display:flex;align-items:baseline;gap:6px;margin-bottom:6px;font-family:'Cormorant Garamond',serif;}
...
.ob-stale{display:none;background:rgba(224,82,82,0.12);border:1px solid rgba(224,82,82,0.4);color:var(--red,#E05252);font-size:11px;font-family:'JetBrains Mono',monospace;padding:5px 8px;border-radius:6px;margin-bottom:6px;text-align:center;}
.ob-stale.on{display:block;}
...
let mountEl = null;
let initialized = false; // double-init guard (split-screen re-init se duplicate listeners rukte hain)
...
// ── Public init ─────────────────────────────────────────────────────
function init(opts = {}) {
if (initialized) return; // listeners sirf ek baar
...
marketStore.onDepth(({ bids, asks }) => {
  if (activeSource !== 'marketStore') return;
  lastDepthAt = Date.now();
  queueRender(bids, asks);
});
...
if (typeof window.chartSplit !== 'undefined') {
  ...
  window.chartSplit.onPane2Depth(({ bids, asks }) => {
    if (activeSource !== 'pane2') return;
    lastDepthAt = Date.now();
    queueRender(bids, asks);
  });
}
...
// Latest-frame-wins throttle: 100ms depth ticks mein se sirf zaroori
// frames paint hote hain; beech ke frames silently drop.
function queueRender(bids, asks) {
pendingFrame = { bids, asks };
...
setTimeout(() => {
  ...
  pendingFrame = null;
  if (frame) renderRows(frame.bids, frame.asks);
}, wait);
}
function checkStaleness() {
const banner = document.getElementById('ob-stale');
if (!banner) return;
const dead = lastDepthAt > 0 && (Date.now() - lastDepthAt) > STALE_AFTER_MS;
banner.classList.toggle('on', dead);
banner.textContent = dead ? '⚠ Order book stale — depth stream disconnected' : '';
}
function render(symbol) {
mountEl.innerHTML = `<div class="ob-panel"> <div class="ob-header"> <span class="ob-header-title">Order Book</span> <span class="ob-header-symbol" id="ob-symbol-label">${formatSymbol(symbol)}</span> </div> <div class="ob-stale" id="ob-stale"></div> <div class="ob-col-labels"><span>Price</span><span>Size ($)</span></div> <div class="ob-body" id="ob-body"><div style="color:var(--muted,#8a8f98);font-size:12px;padding:10px;">Loading order book...</div></div> <div class="ob-pressure" id="ob-pressure"> <div class="ob-pressure-labels"> <span class="ob-pressure-buy-lbl">Buy <span id="ob-pressure-buy-pct">50%</span></span> <span class="ob-pressure-sell-lbl"><span id="ob-pressure-sell-pct">50%</span> Sell</span> </div> <div class="ob-pressure-bar"> <div class="ob-pressure-buy" id="ob-pressure-buy" style="width:50%;"></div> <div class="ob-pressure-sell" id="ob-pressure-sell" style="width:50%;"></div> </div> </div> </div>`;
}
...
// Adaptive precision: 0.01-rounded merged levels ko display pe alag
// dikhata hai (toFixed(1) unhe collapse karke "duplicate rows" dikhata tha).
function priceFmt(p) {
...
// market-store emits each level as {price, qty, total} — total is the
// DOLLAR value (price * qty), summed across all active brokers.
function renderRows(bids, asks) {
const body = document.getElementById('ob-body');
if (!body) return;
if (!bids.length && !asks.length) {
  body.innerHTML = '<div style="color:var(--muted,#8a8f98);font-size:12px;padding:10px;">No depth data yet…</div>';
  return;
}
...
topAsks.forEach(level => {
  ...
  html += `<div class="ob-row ask"><div class="ob-bar" style="width:${pct}%"></div><span class="ob-price ask">${priceFmt(level.price)}</span><span>${formatDollar(level.total)}</span></div>`;
});
const bestBid = topBids[0] ? topBids[0].price : 0;
const bestAsk = topAsks[topAsks.length - 1] ? topAsks[topAsks.length - 1].price : 0;
// Cross-book guard: negative/zero spread display nahi hone denge
const spread = (bestAsk && bestBid && bestAsk > bestBid) ? (bestAsk - bestBid).toFixed(1) : '--';
html += `<div class="ob-spread">${spread} spread</div>`;
topBids.forEach(level => {
  const pct = (level.total / maxTotal * 100).toFixed(0);
  html += `<div class="ob-row bid"><div class="ob-bar" style="width:${pct}%"></div><span class="ob-price bid">${priceFmt(level.price)}</span><span>${formatDollar(level.total)}</span></div>`;
});
body.innerHTML = html;
// Buy/Sell pressure — cumulative bid vs ask DOLLAR value across visible depth.
...
// Needs a <div id="order-book-root"></div> in the layout. No other setup —
// as soon as market-store.js starts receiving depth updates, this renders
// itself automatically (throttled to ~6-7 fps, with a stale banner if the
// depth stream dies for 15s+).
// ══════════════════════════════════════════════════════════════════════════
